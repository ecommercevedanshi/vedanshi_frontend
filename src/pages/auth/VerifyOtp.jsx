...
const VerifyOtp = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    setServerError("");
    try {
      const payload = { email, otpType, otp: values.otp };
      await verifyOtp(payload).unwrap();

      if (otpType === "forgotPassword") {
        navigate("/reset-password", { state: { email } });
      } else {
        await mergeCartAfterLogin({ dispatch, mergeCartApi: mergeCart });
        navigate("/", { replace: true });
        if (from === "checkout") {
          dispatch(openCart());
        }
      ...
      setServerError(err?.data?.message || "Invalid OTP. Please try again.");
      console.log(err);
    }
    setSubmitting(false);
  };
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setServerError(err?.data?.message || "Failed to resend OTP. Please try again.");
      console.log(err);
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-bgMain flex items-center justify-center px-4">
      <div className="w-full max-w-[420px] bg-white border border-borderLight rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] p-6">
        ...
        {serverError && (
          <div className="flex items-start gap-2 bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg px-4 py-3 mb-4">
            <span className="mt-0.5 shrink-0"><AlertTriangle size={18}/></span>
            ...
            <Form className="space-y-4">
              <div>
                <Field
                  name="otp"
                  placeholder="Enter OTP"
                  className="w-full text-center text-lg tracking-[0.3em] border border-borderMedium rounded px-3 py-2"
                />
                <ErrorMessage
                  name="otp"
                  component="div"
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-primary hover:bg-primaryHover text-white py-2 rounded font-medium transition"
              >
                Verify OTP
              </button>
